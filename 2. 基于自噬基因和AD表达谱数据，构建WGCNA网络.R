dataExpr<-read.csv('exprdata.csv',quote='',header=T,row.names=1,check.names=F)
gene<-read.csv('aut_gene.csv',quote='',header=T,check.names=F)
dataExpr<-dataExpr[rownames(dataExpr)%in%gene$id,]
traitData<-read.csv('phe_ad.csv',quote='',header=T,row.names=1,check.names=F)
...
traitData$AD<-ifelse(traitData$type=='AD',1,0)
traitData$normal<-ifelse(traitData$type=='AD',0,1)
...
x<-as.data.frame(x)
data<-x[x$moduleColors=='blue'|x$moduleColors=='brown'|x$moduleColors=='green',]
...
data<-data[,-1:-2]
write.csv(data,'blue.csv',quote=F)
